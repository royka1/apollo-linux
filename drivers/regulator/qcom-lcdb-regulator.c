					...
					u16 addr, u8 value)
{
	...
	u8 pmic_subtype = lcdb->pmic->subtype;
...
static int qpnp_lcdb_save_settings(struct qpnp_lcdb *lcdb)
{
	...
	u16 pmic_subtype = lcdb->pmic->subtype;
...
static int qpnp_lcdb_enable_wa(struct qpnp_lcdb *lcdb)
{
	...
	if (lcdb->pmic->subtype != PM660L_SUBTYPE)
		return 0;
	...
	if (lcdb->pmic->minor == PM660L_V1P1_REV3 &&
		lcdb->pmic->rev2 == PM660L_V1P1_REV4) {
		/*
...
{
	switch (lcdb->pmic->subtype) {
	case PM660L_SUBTYPE:
		if (lcdb->pmic->rev2 < PM660L_V2P0_REV4)
...
static int qpnp_lcdb_hw_init(struct qpnp_lcdb *lcdb)
{
	...
	if (lcdb->sc_irq >= 0 &&
		lcdb->pmic->subtype != PM660L_SUBTYPE) {
		lcdb->sc_count = 0;
...
static int qpnp_lcdb_regulator_probe(struct platform_device *pdev)
{
	...
	lcdb->pmic = qcom_pmic_get(pdev->dev.parent);
	if (IS_ERR_OR_NULL(lcdb->pmic)) {
		struct qcom_spmi_pmic *dummy_pmic;
		pr_debug("PMIC data not ready yet, using dummy\n");

		dummy_pmic = devm_kzalloc(&pdev->dev, sizeof(*dummy_pmic), GFP_KERNEL);
		if (!dummy_pmic)
			return -ENOMEM;

		dummy_pmic->subtype = 11; /* 11 is de numerieke waarde voor PM8150L_SUBTYPE */
		lcdb->pmic = dummy_pmic;
	}

	lcdb->dev = &pdev->dev;
	lcdb->pdev = pdev;
	platform_set_drvdata(pdev, lcdb);
...
module_platform_driver(qpnp_lcdb_regulator_driver);
