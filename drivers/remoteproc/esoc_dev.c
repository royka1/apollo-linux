...
static void esoc_udev_handle_clink_req(enum esoc_req req, struct esoc_eng *eng)
{
	int err;
	u32 clink_req;
...
static void esoc_udev_handle_clink_evt(enum esoc_evt evt, struct esoc_eng *eng)
{
	int err;
	u32 clink_evt;
	struct esoc_clink *esoc_clink = eng->esoc_clink;
						...
						unsigned long arg)
{
	...
	case ESOC_NOTIFY:
		...
		break;
	/*
	 * These two are polled once a second for the lifetime of the system by
	 * the request engine. Logging them at info level buries every other
	 * message in the ring buffer, which is exactly the history needed when
	 * something does go wrong, so keep them at debug.
	 */
	case ESOC_GET_STATUS:
		clink_ops->get_status(&status, esoc_clink);
		pr_debug("esoc-mdm: ESOC_GET_STATUS: Sending the status from esoc: %u\n",
			 status);
		put_user(status, (unsigned int __user *)uarg);
		break;
	case ESOC_GET_ERR_FATAL:
		clink_ops->get_err_fatal(&status, esoc_clink);
		pr_debug("esoc-mdm: ESOC_GET_ERR_FATAL: Sending err_fatal status from esoc: %u\n",
			 status);
		put_user(status, (unsigned int __user *)uarg);
		break;
...
int esoc_clink_add_device(struct device *dev, void *dummy)
{
	...
	esoc_udev->dev = device_create(esoc_class, &esoc_clink->dev,
				       ...
				       esoc_clink, "subsys_%s", esoc_clink->name);
...
int esoc_dev_init(void)
{
	...
	esoc_class = class_create("esoc-dev");
...
EXPORT_SYMBOL(esoc_clink_add_device);
EXPORT_SYMBOL(esoc_clink_del_device);
MODULE_LICENSE("GPL");
