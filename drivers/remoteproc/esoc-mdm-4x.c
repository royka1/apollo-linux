...
static void mdm_trace_state(struct mdm_ctrl *mdm, const char *tag)
{
	pr_info("esoc-mdm: %s ap2mdm_status=%d ap2mdm_errfatal=%d mdm2ap_status=%d mdm2ap_errfatal=%d ready=%d init=%d\n",
		tag,
...
static int mdm_cmd_exe(enum esoc_cmd cmd, struct esoc_clink *esoc)
{
	...
	switch (cmd) {
	case ESOC_PWR_ON:
		mdm_trace_state(mdm, "before ESOC_PWR_ON");
		if (esoc->auto_boot) {
			/*
		...
		mdm_enable_irqs(mdm);
		mdm_trace_state(mdm, "after ESOC_PWR_ON");
		break;
	case ESOC_PWR_OFF:
	...
	case ESOC_EXIT_DEBUG:
		/*
		 * Drive STATUS low before deasserting ERR_FATAL so the modem
		 * sees a clean 0→1 transition when ESOC_PWR_ON raises it
		 * after the warm reset.  Without this, the modem stays in
		 * debug/crash mode and ignores the subsequent SPMI reset.
		 */
		esoc_mdm_log("ESOC_EXIT_DEBUG: Setting AP2MDM_STATUS = 0\n");
		gpio_set_value(MDM_GPIO(mdm, AP2MDM_STATUS), 0);
		/* Let the modem detect the STATUS edge before reset */
...
static void mdm2ap_status_check(struct work_struct *work)
{
	...
	mdm_trace_state(mdm, "status-check timeout");
...
static void mdm_status_fn(struct work_struct *work)
{
	...
	mdm_trace_state(mdm, "mdm_status_fn");
...
static void mdm_get_restart_reason(struct work_struct *work)
{
	...
	do {
		ret = qcom_sysmon_get_reason(mdm->esoc->rproc_sysmon, sfr_buf,
					    sizeof(sfr_buf));
		if (!ret) {
...
static void mdm_notify(enum esoc_notify notify, struct esoc_clink *esoc)
{
	...
	case ESOC_IMG_XFER_DONE:
		mdm_trace_state(mdm, "notify XFER_DONE");
		if (gpio_get_value(MDM_GPIO(mdm, MDM2AP_STATUS)) ==  0) {
			esoc_mdm_log(
		...
		break;
	case ESOC_BOOT_DONE:
		mdm_trace_state(mdm, "notify BOOT_DONE");
		mdm->needs_hard_reset = false;
...
static irqreturn_t mdm_status_change(int irq, void *dev_id)
{
	...
	mdm_trace_state(mdm, "status IRQ");
	/*
...
static irqreturn_t mdm_pblrdy_change(int irq, void *dev_id)
{
	...
	mdm_trace_state(mdm, "pblrdy IRQ");
	if (mdm->init) {
		mdm->init = 0;
...
static void mdm_remove(struct platform_device *pdev)
{
...
EXPORT_SYMBOL(ipc_log);
EXPORT_SYMBOL(mdm_wait_for_status_low);
EXPORT_SYMBOL(mdm_disable_irqs);
