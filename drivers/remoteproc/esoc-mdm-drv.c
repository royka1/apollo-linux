...
/*
 * How long to wait for pon_done before giving up on a boot.
 *
 * pon_done is completed by ESOC_RUN_STATE, which for a non-auto_boot esoc only
 * ever arrives when userspace sends ESOC_BOOT_DONE. Waiting for that forever
 * (the vendor uses INT_MAX) means any userspace hiccup wedges remoteproc for
 * good: the wait is uninterruptible and it is held across rproc->lock, so
 * every later start/stop blocks behind it and no signal can break it out.
 * A generous bound turns that into an error the caller can retry.
 *
 * A healthy boot is ~30 s (PCIe recovery, then ~85 MB of firmware over
 * Sahara), so this leaves a wide margin.
 */
#define PON_DONE_TIMEOUT_MS	(180 * 1000)
#define ESOC_MAX_PON_TRIES	5
/*
 * Panicking the AP because an external modem would not boot is not a
 * reasonable default on a phone: the modem is a peripheral here, and taking
 * the whole system down loses the log that would explain the failure. Leave
 * the modem alone and report the error to whoever asked for the boot.
 */
...
static void mdm_ssr_fn(struct work_struct *work)
{
	...
	/*
	 * Drive client crash hooks (e.g. MHI RDDM collection) as soon as we
	 * know the modem has gone into CRASH, before waiting for any ESOC
	 * boot-done synchronization.  On upstream boot paths that never send
	 * ESOC_BOOT_DONE, ssr_ready would otherwise block forever here and the
	 * BHIe-accessible crash dump would be lost.
	 */
	esoc_client_link_power_off(mdm_drv->esoc_clink, ESOC_HOOK_MDM_CRASH);

	/*
	 * Wait for pon to complete, but with a bounded timeout so we don't
	 * pin a worker thread forever if the modem crashed before it reached
	 * PON_SUCCESS (e.g. no userspace BOOT_DONE notifier is running).
	 */
	ret = wait_for_completion_timeout(&mdm_drv->ssr_ready,
					  msecs_to_jiffies(5000));
	if (!ret || mdm_drv->pon_state != PON_SUCCESS) {
...
static int mdm_rproc_powerup(struct rproc *rproc)
{
	...
	struct mdm_drv *mdm_drv = esoc_get_drv_data(esoc_clink);
	const struct esoc_clink_ops * const clink_ops = esoc_clink->clink_ops;
	int timeout = PON_DONE_TIMEOUT_MS;
	u8 pon_trial = 0;
	...
	do {
		...
		if (mdm_drv->mode == PWR_OFF) {
			...
			if (mdm_dbg_stall_cmd(ESOC_PWR_ON))
				return -EBUSY;
			/*
			 * Clear the hard-reset flag so a graceful
			 * restart after a clean shutdown uses the
			 * normal warm reset.
			 */
			((struct mdm_ctrl *)get_esoc_clink_data(esoc_clink))->needs_hard_reset = false;
...
static int esoc_ssr_probe(struct esoc_clink *esoc_clink, struct esoc_drv *drv)
{
	int ret;
	...
	if (IS_ERR(data)) {
		dev_err(&esoc_clink->dev, "esoc match data failed: %ld\n", PTR_ERR(data));
		return PTR_ERR(data);
	}
...
static int esoc_ssr_remove(struct esoc_clink *esoc_clink, struct esoc_drv *drv)
{
	int ret;
	struct mdm_drv *mdm_drv;
...
EXPORT_SYMBOL(mdm_drv_init);
EXPORT_SYMBOL(mdm_drv_exit);
EXPORT_SYMBOL(esoc_set_boot_fail_action);
EXPORT_SYMBOL(esoc_set_n_pon_tries);
